...
Public Class HSPI
  ...
  '
  Public GetLiveWeatherThread As Thread
  Public GetForecastWeatherThread As Thread
  Public GetAlertsWeatherThread As Thread
...
#Region "HSPI - Base Plugin API"
          ...
          Case "WeatherBug State"
            ...
            Select Case dev_value
              Case -1
                ...
                WriteMessage("Weatherbug checking is enabled.", MessageType.Informational)
              ...
              Case -2
                ...
                WriteMessage("Weatherbug checking is disabled.", MessageType.Informational)
  ...
  Public Function InitIO(ByVal port As String) As String Implements HomeSeerAPI.IPlugInAPI.InitIO
    ...
    Try
      ...
      ' Update the API Keys
      '
      gAPIKeyPrimary = hs.GetINISetting("API", "KeyPrimary", "", gINIFile)
      gAPIKeySecondary = hs.GetINISetting("API", "KeySecondary", "", gINIFile)

      '
      ' Device Image
      '
      gDeviceImage = CBool(hs.GetINISetting("Options", "DeviceImage", gDeviceImage, gINIFile))

      '
      ' Refresh the Access Token
      '
      WeatherBugAPI = New hspi_weatherbug_api
      ...
      If bDBInitialized = True Then
        ...
        If bVersionChanged Then
          ...
          hspi_devices.UpdateRootDevices()
        End If
        ...
        '
        GetLiveWeatherThread = New Thread(New ThreadStart(AddressOf CheckWeatherThread))
        GetLiveWeatherThread.Name = "GetLiveWeather"
        GetLiveWeatherThread.Start()

        WriteMessage(String.Format("{0} Thread Started", GetLiveWeatherThread.Name), MessageType.Debug)

        GetForecastWeatherThread = New Thread(New ThreadStart(AddressOf CheckForecastThread))
        GetForecastWeatherThread.Name = "GetForecastWeather"
        GetForecastWeatherThread.Start()

        WriteMessage(String.Format("{0} Thread Started", GetForecastWeatherThread.Name), MessageType.Debug)

        GetAlertsWeatherThread = New Thread(New ThreadStart(AddressOf CheckAlertsThread))
        GetAlertsWeatherThread.Name = "GetAlertsWeather"
        GetAlertsWeatherThread.Start()

        WriteMessage(String.Format("{0} Thread Started", GetAlertsWeatherThread.Name), MessageType.Debug)
  ...
  Public Sub ShutdownIO() Implements HomeSeerAPI.IPlugInAPI.ShutdownIO
    ...
    Try
      ...
      If GetLiveWeatherThread.IsAlive = True Then
        GetLiveWeatherThread.Abort()
      End If

      If GetForecastWeatherThread.IsAlive = True Then
        GetForecastWeatherThread.Abort()
      End If

      If GetAlertsWeatherThread.IsAlive = True Then
        GetAlertsWeatherThread.Abort()
      End If
...
#Region "HSPI - Speak Proxy"
  ...
  Public Sub SpeakIn(device As Integer, txt As String, w As Boolean, host As String) Implements HomeSeerAPI.IPlugInAPI.SpeakIn
    ...
    '
    'txt = hspi_plugin.WeatherForecastReplacmentVariables("Station1", txt)
    'txt = hspi_plugin.WeatherAlertsReplacementVariables("Station1", txt)
    'txt = hspi_plugin.ReplaceVariables("Station1", txt)
    'txt = hspi_plugin.SpeachReplacementVariables(txt)

    'hs.SpeakProxy(device, txt, w, host)
...
#Region "HSPI - Public Sub/Functions"
  ...
  '
  ' Determines if API Key is validd
  '
  Public Function ValidAPIKey() As Boolean

    Try

      Return WeatherBugAPI.ValidAPIKey()
  ...
  Public Function SearchLocations(ByVal searchString As String) As ArrayList
    Return hspi_plugin.SearchLocations(searchString)
  End Function
  ...
  Public Function SearchStationIds(ByVal searchString As String) As ArrayList
    Return hspi_plugin.SearchStationIds(searchString)
  End Function
  ...
  Public Sub RefreshStation(ByVal strStationNumber As String)
    hspi_plugin.RefreshStation(strStationNumber)
  End Sub
  ...
  Public Function GetStations() As SortedList
    Return hspi_plugin.GetStations
  End Function
  ...
  Public Function GetStation(ByVal strStationNumber As String) As Hashtable
    Return hspi_plugin.GetStation(strStationNumber)
  End Function

  ''' <summary>
  ''' Returns supported weather keys
  ''' </summary>
  ''' <param name="strKeyType"></param>
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function GetWeatherKeys(ByVal strKeyType As String) As String()
    Return hspi_plugin.GetWeatherKeys(strKeyType)
  End Function
  ...
  Public Function CreateWeatherBugDevice(ByVal strDeviceString As String, _
    ...
    Return hspi_devices.CreateWeatherBugDevice(strDeviceString, bUpdateOnly)
  End Function
  ...
  Public Function GetForecast(ByVal strStationNumber As String) As Hashtable
    Return hspi_plugin.GetForecast(strStationNumber)
  End Function
  ...
  Public Function GetDaysOfWeek() As ArrayList
    Return hspi_plugin.GetDaysOfWeek()
  End Function

  ''' <summary>
  ''' Returns alerts sorted list
  ''' </summary>
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function GetAlerts() As SortedList
    Return hspi_plugin.Alerts
  End Function
  ...
  Public Function QueryDatabase(ByVal strSQL As String) As DataSet
    ...
    '
    strMessage = "Entered QueryDatabase() subroutine.  "
    ...
    Try
      ...
      ' Execute query
      '
      If DBConnectionMain.State = ConnectionState.Open Then
        Return Database.QueryDatabase(strSQL)
      Else
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function ConvertDateTimeToEpoch(ByVal dateTime As DateTime) As Long

    Return Database.ConvertDateTimeToEpoch(dateTime)
  ...
  ''' <returns></returns>
  ''' <remarks></remarks>
  Public Function ConvertEpochToDateTime(ByVal epochTicks As Long) As DateTime

    Return Database.ConvertEpochToDateTime(epochTicks)
